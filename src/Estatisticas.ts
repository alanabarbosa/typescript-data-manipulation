...
const filtrarValor = (transacao: Transacao): transacao is TransacaoValor => transacao.valor !== null;
...
export default class Estatisticas {
    ...
    private setPagamento = () => countBy(this.transacoes.map(({pagamento}) => pagamento));  

    private setStatus = () => countBy(this.transacoes.map(({status}) => status));
